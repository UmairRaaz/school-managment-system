...
const AdminDashboard = () => {
  ...
  const data = [
    { category: "Financial", name: "Total Amount", value: totalAmount, date: new Date(), icon: <FaSackDollar /> },
    { category: "Student", name: "Students", value: totalStudents, date: new Date(), icon: <FcBusinessman /> },
    { category: "Student", name: "Present", value: totalAttendanceToday, date: new Date(), icon: <FcVoicePresentation /> },
    { category: "Staff", name: "Teachers", value: totalTeachers, date: new Date(), icon: <FcAssistant /> },
    { category: "Financial", name: "Received Amount", value: totalAmount, date: new Date(), icon: <GiTakeMyMoney /> },
    { category: "Financial", name: "Pending Amount", value: totalAmount, date: new Date(), icon: <MdOutlinePending /> },
    { category: "Financial", name: "Paid Students", value: totalAmount, date: new Date(), icon: <FcPaid /> },
    { category: "Financial", name: "Unpaid Students", value: totalAmount, date: new Date(), icon: <FaUserSlash /> },
    { category: "Student", name: "Result", value: totalAmount, date: new Date(), icon: <FcSimCardChip /> },
    { category: "Student", name: "Passed Student", value: totalAmount, date: new Date(), icon: <FcLike /> },
    { category: "Student", name: "Failed Student", value: totalAmount, date: new Date(), icon: <FcDislike /> },
    { category: "Notification", name: "Class Notification", value: totalAmount, date: new Date(), icon: <IoIosNotifications /> },
    { category: "Notification", name: "Public Notification", value: totalAmount, date: new Date(), icon: <PiMicrophoneStageFill /> },
    { category: "Student", name: "Attendance", value: totalAmount, date: new Date(), icon: <FaBoxes /> },
  ];
  ...
  const applyTimeFilter = (items) => {
    switch (timeFilter) {
      case "Daily":
        return items.filter(item => item.date.toDateString() === today.toDateString());
      case "Weekly":
        const weekStart = new Date(today.setDate(today.getDate() - today.getDay()));
        const weekEnd = new Date(today.setDate(today.getDate() - today.getDay() + 6));
        return items.filter(item => item.date >= weekStart && item.date <= weekEnd);
      case "Monthly":
        return items.filter(item => item.date.getMonth() === today.getMonth() && item.date.getFullYear() === today.getFullYear());
      case "Yearly":
        return items.filter(item => item.date.getFullYear() === today.getFullYear());
      case "Date-wise":
  ...
  <div className="flex flex-wrap gap-2 mb-4 md:mb-0">
    <button onClick={() => setFilter("All")} className="py-2 px-8 bg-blue-500 text-white rounded-lg ">All</button>

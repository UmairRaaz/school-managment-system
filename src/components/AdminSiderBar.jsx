...
import { FaUserGraduate ,FaAngleRight, FaHome, FaTachometerAlt, FaPlus, FaBoxes, FaShoppingCart, FaLifeRing, FaUsers, FaChevronLeft, FaChevronRight, FaAngleDown, FaAngleLeft, FaCog } from 'react-icons/fa';

const Sidebar = () => {
    const [isOpen, setIsOpen] = useState(true);
    const [showDropdown, setShowDropdown] = useState(Array(8).fill(false)); // Initialize with false for each link
    ...
    const navLinks = [
        ...
        { href: "/", icon: FaBoxes, label: "Attendance", color: "blue", tooltip: "Attendance", hasDropdown: true, dropdownLinks: [
            { href: "/admin/all-orders", label: "All Orders" }
        ]},
        { href: "/", icon: FaShoppingCart, label: "Result", color: "blue", tooltip: "Result", hasDropdown: true, dropdownLinks: [
            { href: "/admin/all-supports", label: "All Support" }
        ]},
        { href: "/", icon: FaLifeRing, label: "Fees", color: "blue", tooltip: "Fees", hasDropdown: true, dropdownLinks: [
            { href: "/admin/all-users", label: "All Users" }
        ]},
        { href: "/", icon: FaLifeRing, label: "Notification", color: "blue", tooltip: "Notification", hasDropdown: true, dropdownLinks: [
            { href: "/admin/all-users", label: "All Users" }
        ]},

        { href: "/api/auth/signout", icon: FaUsers, label: "Logout", color: "red", tooltip: "Logout" }
    ];

    return (
        <div className={`h-screen fixed mt-20 pb-40  left-0 top-0 bg-white text-black ${isOpen ? 'w-64' : 'w-20'} transition-all duration-300 flex flex-col overflow-y-auto`}>
            <div className="py-4 px-6 flex justify-between items-center">
                <Link href="/" className={`flex justify-center items-center ${isOpen ? 'md:flex' : 'md:hidden'} py-2.5  hover:text-black`}>
                    ...
                    <span className={`text-xl text-nowrap font-bold transition-all duration-300 ${!isOpen && 'hidden'}`}>Moon School</span>
                </Link>
            ...
            <nav className="flex flex-col flex-1 px-2 space-y-2 gap-3">
                {navLinks.map((link, index) => (
                    <div className="relative group" key={index}>
                        {link.hasDropdown ? (
                            <>
                                <div className="flex items-center py-2.5 px-4 rounded-lg transition duration-200 bg-white hover:bg-white cursor-pointer" onClick={() => toggleDropdown(index)}>
                                    <link.icon className="w-6 h-6 mr-3 text-blue-800" />
                                    <span className={`transition-all duration-300 ${!isOpen && 'hidden'} group-hover:text-blue-500`}>{link.label}</span>
                        ...
                        <div className="absolute bottom-full mb-2 hidden group-hover:flex justify-center w-full">
                            <div className="relative bg-blue-600 text-white text-xs rounded py-1 px-3 shadow-lg">
                                ...
                                <div className="absolute w-3 h-3 left-1/2 transform -translate-x-1/2 top-full">
                                    <svg className="fill-current text-blue-600" viewBox="0 0 100 100" width="100%" height="100%">

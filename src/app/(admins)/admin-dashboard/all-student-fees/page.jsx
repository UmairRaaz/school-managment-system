...
const AllStudentsFees = () => {
  ...
  const filteredFees = fees.filter(fee => {
    const matchesClass = filterClass ? fee.studentId.CurrentClass === filterClass : true;
    const matchesStatus = filterStatus ? (filterStatus === "Paid" ? fee.isPaid : !fee.isPaid) : true;
    const matchesSearch = fee.studentId.Name.toLowerCase().includes(searchTerm.toLowerCase()) || fee.studentId.SID.toLowerCase().includes(searchTerm.toLowerCase());
          ...
          <tbody className="text-gray-700 text-xs">
            {filteredFees.map((fee) => (
              ...
              >
                ...
                <td className="py-2 px-4 text-left">{fee.studentId.Name}</td>
                <td className="py-2 px-4 text-left">{fee.studentId.CurrentClass}</td>
                <td className="py-2 px-4 text-left">{fee.studentId.Section}</td>
                <td className="py-2 px-4 text-left">{fee.studentId.MobileNumber}</td>
                <td className="py-2 px-4 text-left">{new Date(fee.date).toISOString().split('T')[0]}</td>
                <td className="py-2 px-4 text-left">
                  <select
